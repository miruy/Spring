...
/* boolean agree : 커맨드 객체 값의 존재 여부, 초기화 값 : 없음 -> 값이 정상적으로 들어갔다면 : true */
	...
	// +추가 Validator인터페이스 적용위해 코드 수정 
	@RequestMapping(value="/register/step3", method=RequestMethod.POST)        //에러도 함께 전달
	public String handleStep3(@ModelAttribute("formData")RegisterRequest regReq, Errors errors) {
		
		//유효성 검증 및 예외 메세지 처리 메서드 호출
		//스프링은 자동으로 전달받은 regReq객체와 연결된 에러객체 생성 후 전달
		new RegisterRequestValidator().validate(regReq, errors);
		
		if(errors.hasErrors()) {	//validate() 에러 검증 메서드 실행 중 한번이라도 에러 발생 했을 경우
			return "register/step2";
		}
		
		//validate() 에러 검증 메서드 실행 중 한번이라도 에러가 발생하지 않았을 경우 
		try {
			memberRegisterService.regist(regReq);	//회원 등록 
			return "register/step3";
		} catch (AlreadyExistingMemberException e) {
			errors.rejectValue("email", "duplicate");	//위의 회원 등록 시 일어날 수 있는 예외 : 이메일 중복 에러메세지 errors객체에 저장
			return "register/step2";
		}
	}
